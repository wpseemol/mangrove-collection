...
export default function Thumbnail({ form }: { form: AddProductFormType }) {
    ...
    useEffect(() => {
        if (fileRejections[0]?.errors[0]?.code === 'file-invalid-type') {
            form.setError('thumbnail', {
                type: 'manual',
                message: `You selected ${fileRejections[0]?.file.type}. A image can thumbnail.`,
            });
        } else if (fileRejections[0]?.errors[0]?.code === 'too-many-files') {
            form.setError('thumbnail', {
                type: 'manual',
                message: `You selected ${fileRejections?.length} files. A single image file can thumbnail.`,
            });
        }
        // image upload unction
    }, [form, acceptedFiles]);
