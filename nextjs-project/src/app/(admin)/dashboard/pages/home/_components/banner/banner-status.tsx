...
export default function BannerStatus({ form, loading, data }: SlideStatusProps) {
        ...
        useEffect(() => {
                ...
                const dbDataVal = dbData.map((banner) => ({
                        title: banner.title,
                        imageUrl: banner.imageUrl,
                        linkStatus: banner.linkStatus,
                        linkTarget: banner.linkTarget,
                }));

                const formDataVal = form.getValues().banners.map((banner) => ({
                        title: banner.title,
                        imageUrl: banner.imageUrl,
                        linkStatus: banner.linkStatus,
                        linkTarget: banner.linkTarget,
                ...
                // Sort properties to ensure consistent JSON stringify
                const sortObject = <T extends Record<string, any>>(
                        obj: T
                ): T => {
        ...
        const hasErrors = !!errors?.banners;
        ...
        return (
                <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700 px-4 pb-4">
                        ...
                        <button
                                type="submit"
                                disabled={
                                        currentLoading || hasErrors || isDisable
                                }
