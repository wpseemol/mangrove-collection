...
export default function BannerImages({ data }: { data: string }) {
    ...
    const form = useForm<BannersFormData>({
        ...
        defaultValues: {
            banners: slides.filter(
                (slide) =>
                    slide.type === "right-top" || slide.type === "right-bottom"
            ),
        },
    });
    ...
    const onSubmit = async (data: BannersFormData) => {
        const response = await updateHeroBannerImages(JSON.stringify(data));
        if (!response.success) {
        ...
        if (response.success) {
            toast.success(response.message || "Banner Image add Successful.");
            router.refresh();
            return;
        }
    };
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
            {fields.map((field, index) => (
                ...
                >
                    ...
                    <BannerStatus form={form} loading={loading} data={data} />
                </div>
            ))}
        </form>
    );
