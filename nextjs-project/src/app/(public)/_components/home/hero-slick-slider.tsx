...
export default function HeroSlickSlider({
    ...
    const sliderContent = JSON.parse(sliderData) as BannerSliderType[];
                    ...
                    <div
                        className={`${
                            currentIndex === i
                                ...
                                : "bg-white/90"
                        } md:w-5 md:h-2 w-3 h-1 duration-200`}
                    ></div>
                </div>
            );
        },
...
function CustomSlide({ slidItem }: { slidItem: BannerSliderType }) {
    const { scrollY } = useScroll();
    const scale = useTransform(scrollY, [0, 750], [1, 2]);

    if (slidItem.linkStatus) {
        return (
            <Link href={`${slidItem.linkTarget}`}>
