    ...
    return (
        <section className="container mx-auto md:pb-10 pb-5 px-2 md:px-0">
            ...
            {categories.length > 0 && (
                <motion.div
                    className={`${
                        categories.length < 8 ? "flex justify-center " : "grid"
                ...
                >
                    {categories.map((category: Category) => (
                        <Card
                            key={category.id}
                            className="bg-card py-4 sm:px-6 px-1 group
                            hover:text-primary-foreground duration-300 rounded border-neutral-900/15 overflow-hidden 
                            md:h-[175px] h-[160px] w-fit justify-self-center flex flex-col justify-center items-center 
