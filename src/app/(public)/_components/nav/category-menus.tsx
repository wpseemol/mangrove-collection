...
export default async function CategoryMenus() {
    ...
    return (
        <ul className="bg-white dark:bg-neutral-800 group-hover/category:md:h-fit group-hover/category:block md:block md:h-0 hidden md:absolute top-9 md:rounded-b rounded-r border-t-0 md:min-w-72 md:w-fit w-screen duration-150 overflow-hidden  origin-top text-lg text-nowrap md:shadow-lg mt-2 -ml-2 z-20 group-hover/category:animate-flip-down animate-duration-1000">
            {categories.map((category) => (
                <li
                    key={category?.id}
                    className="hover:text-primary-foreground  px-2  md:border-b-black/10 first:border-t-black/10 w-full 
                    group-hover/category:h-fit h-0 animate-duration-700 group/sub border-b-border border-black/10 border border-r-0 border-l-0 md:bg-secondary text-secondary-foreground duration-200 ">
                    <CustomLink
